...
class Category(models.Model):
    ...
    CharField 指定了分类名 name 数据类型，CharField是字符型，
    CharField 的 max_length 参数指定其最大长度，超过这个长度的分类名就不能被存入数据库。
    当然 django 还为我们提供了多种其他的数据类型，如日期时间类型DateTimeField、整数类型IntegerField等等。
    django的全部类型可查看文档: https://docs.djangoproject.com/en/2.2/ref/models/fields/#field-types
    """
...
class Post(models.Model):
    ...
    # 文章正文，使用TextField ,存储比较短的字符串可以使用CharField，但是对于文章的正文来说可能会有一大段文本，因此使用TextField来存储打断文本
    ...
    # 即一对多的关联关系。且自 django 2.0 以后，ForeignKey 必须传入一个 on_delete 参数
    # 用来指定当关联的数据被删除时，被关联的数据的行为，我们这里假定当某个分类被删除时，该分类下全部文章也同时被删除，
    # 因此 使用 models.CASCADE 参数，意为级联删除。
